...
backup_checkmd5()
{
  ...
  wget -q --tries=3 --wait=180 --timeout=60 -q $FTPURL/$FTPPATH/$IMGNAME -O - | $jetcat -f 1000 -p $IMGSIZE 2> $PROGRESSFILE | md5sum | awk '{print $1}' > $TMPFILE
...
dd_backup()
{
   ...
   # todo check if the partitions is ntfs
   declare diskname=`basename  $SOURCEDISK`
   declare disksize=`getdevicesize $diskname`

   umount $SOURCEDISK
   mkdir -p "/mnt/$diskname"
   mount.ntfs $SOURCEDISK "/mnt/$diskname"
   if [ $? != 0 ];then
		tasklog "partition is not ntfs"
		return 1
   fi
#   echo "ntfs mounted "
#   ls /mnt/$diskname -l
   ...
   ntfsclone --save-image -o - $SOURCEDISK 2>/dev/null | gzip | tee $MD5PIPE > $TRANPIPE &
